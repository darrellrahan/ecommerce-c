...
void tambahProduk() {
    ...
    printf("\nProduk berhasil ditambahkan dengan ID %d ke file %s\n", produkbaru.id, PRODUCT_DATABASE);
...
void hapusProduk()
{
    ...
    if (!file || !tempFile)
    {
        ...
        if (file) fclose(file); // Menutup file jika berhasil dibuka
        if (tempFile) fclose(tempFile); // Menutup file jika berhasil dibuka
        return;
    }
    ...
    // Salin ke file sementara dengan mengecualikan produk yang dihapus
    while (fgets(buffer, sizeof(buffer), file))
    {
        sscanf(buffer, "%d,%[^,],%[^,],%d,%d", &produk.id, produk.nama, produk.kategori, &produk.harga, &produk.stock);
        if (produk.id != id)
    ...
    // Menutup file sebelum melakukan operasi lebih lanjut
    fclose(file);
    fclose(tempFile);
...
void rekapPenjualan()
{
    ...
    // Membaca data keuntungan ke dalam array
    struct CartDatabase penjualanEntries[25];
    ...
    // Membaca data produk ke dalam array
    struct Produk produkEntries[25];
    int produkCount = 0;
    char line[200];
    ...
    // Sinkronisasi dan tampilkan hasil
