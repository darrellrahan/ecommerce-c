...
{	
    ...
    char buffer[256];
    struct Produk listProduk[jumlahProduk];
    int index = 0;

    // Baca semua produk dari database
...
void checkout(int totalHarga)
{
    ...
    struct Produk cart[jumlahProduk];
    struct CartDatabase cartDatabase[jumlahProduk];
    getCart(cart, cartDatabase);

    struct CartDatabase newCartDatabase[jumlahProduk];
    ...
    for (int i = 0; i < (sizeof(cartDatabase) / sizeof(cartDatabase[0])); i++)
    {
        if (cartDatabase[i].email[0] != '\0')
        {
            ...
            {
                // Menyimpan barang yang sudah di-checkout ke database keuntungan
                ...
                // Menyimpan email dan ID barang ke database keuntungan
